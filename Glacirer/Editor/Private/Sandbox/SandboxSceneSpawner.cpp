...
#include "Basics/Objects/SpotLight.h"
 ...
 namespace Editor
 {
    namespace Sandbox
    {
        ...
        void SandboxSceneSpawner::Spawn(World& world)
        {
            glm::vec3 cameraInitialPosition(-12.f, 11.f, 12.f);
            glm::vec3 cameraInitialRotation(-25.f, -47.f, 0.f);
            std::shared_ptr<FlyingCamera> camera = world.Spawn<FlyingCamera>(cameraInitialPosition, cameraInitialRotation);
            camera->DisablePilotMode();
            camera->SetName("Camera");

            SpawnCrates(world);

            auto cube = world.Spawn<Cube>(glm::vec3(0.f, 5.f, 0.f));
            ...
            anotherMaterial->SetTexture("u_Diffuse", ResourceManager::LoadTexture(SANDBOX_RESOURCES_PATH + "Textures/FancyPigeon.png", "Pigeon", pigeonTextureSettings, true), 1);
            ...
            auto sphere = world.Spawn<Sphere>(glm::vec3(-4.f, 1.f, 2.f));
            ...
            auto postProcessing = world.Spawn<PostProcessing>();
            postProcessing->SetName("PostProcessing");

            auto skybox = world.Spawn<Skybox>();
            skybox->SetName("Skybox");
            skybox->SetDefaultSky();
        }

        void SandboxSceneSpawner::SpawnCrates(World& world)
        {
            ...
            crateMaterial->SetTexture("u_Diffuse", ResourceManager::LoadTexture(SANDBOX_RESOURCES_PATH + "Textures/Container_Diff.png", "Container_Diffuse", TextureSettings{false}, true), 1);
            crateMaterial->SetTexture("u_Specular", ResourceManager::LoadTexture(SANDBOX_RESOURCES_PATH + "Textures/Container_Spec.png", "Container_Specular", TextureSettings{false, false}, true), 2);
            ...
            for(int x = 0; x < 3; x++)
            {
                for(int z = 0; z < 3; z++)
                {
                    ...
                    auto crate = world.Spawn<Cube>(position);
        ...
        void SandboxSceneSpawner::SpawnBridge(World& world)
        {
            auto bridgeMaterial = ResourceManager::CreateMaterial("M_Bridge");
            bridgeMaterial->SetColor("u_Color", glm::vec4(0.f));
            bridgeMaterial->SetTexture("u_Diffuse", ResourceManager::LoadTexture(SANDBOX_RESOURCES_PATH + "Textures/Atlas04_Diff.png", "T_Bridge_Diffuse", TextureSettings{false}), 1);

            auto bridgeModel = ResourceManager::LoadModel(SANDBOX_RESOURCES_PATH + "Models/Bridge.fbx", "Bridge");

            auto bridge = world.Spawn<Model>(glm::vec3(0.f, 0.f, -20.f));
            bridge->Setup(bridgeModel, bridgeMaterial);
            bridge->SetName("Bridge");
        }

        void SandboxSceneSpawner::SpawnWarrior(World& world)
        {
            auto warriorMaterial = ResourceManager::CreateMaterial("M_Liz");
            warriorMaterial->SetColor("u_Color", glm::vec4(0.f));
            warriorMaterial->SetTexture("u_Diffuse", ResourceManager::LoadTexture(SANDBOX_RESOURCES_PATH + "Textures/liz/T_Liz_Diffuse.png", "T_Liz_Diffuse", TextureSettings{false}), 0);
            warriorMaterial->SetTexture("u_Specular", ResourceManager::LoadTexture(SANDBOX_RESOURCES_PATH + "Textures/liz/T_Liz_Specular.png", "T_Liz_Specular", TextureSettings{false}), 1);
        
            auto warriorModel = ResourceManager::LoadModel(SANDBOX_RESOURCES_PATH + "Models/PigeonsAttack_Liz.fbx", "Liz");

            glm::vec3 spawnPosition{0.f, -0.5f, -4.f};
            glm::vec3 spawnRotation{0.f, -31.f, 0.f};
            glm::vec3 spawnScale{0.25f};

            auto warrior = world.Spawn<Model>(spawnPosition, spawnRotation, spawnScale);
            warrior->Setup(warriorModel, warriorMaterial);
            warrior->SetName("Warrior");
        }

        void SandboxSceneSpawner::SpawnTransparentObjects(World& world)
        {
            auto flowerMaterial = ResourceManager::CreateMaterial("M_Flower");
            flowerMaterial->SetColor("u_Color", glm::vec4(0.f));
            flowerMaterial->SetTexture("u_Diffuse", ResourceManager::LoadTexture(SANDBOX_RESOURCES_PATH + "Textures/Flower.png", "T_Flower", TextureSettings{true}, true), 1);
            flowerMaterial->SetRenderingMode(MaterialRenderingMode::AlphaCutout);
        
            auto flowerQuad = world.Spawn<Quad>(glm::vec3(-2.f, 0.f, -2.f), glm::vec3{0.f, -90.f, 0.f});
            ...
            windowMaterial->SetTexture("u_Diffuse", ResourceManager::LoadTexture(SANDBOX_RESOURCES_PATH + "Textures/TransparentWindow.png", "T_Window", TextureSettings{true}, true), 1);
            ...
            for(int i = 0; i < 5; i++)
            {
                ...
                auto windowQuad = world.Spawn<Quad>(windowPosition, windowRotation);
                windowQuad->SetName("Window" + std::to_string(i));
                windowQuad->SetMaterial(windowMaterial);
            }
        }

        void SandboxSceneSpawner::SpawnFloor(World& world)
        {
            glm::vec3 floorPosition{4.f, -0.75f, 4.f};
            glm::vec3 floorRotation{0.f};
            glm::vec3 floorScale{44.f, 0.5f, 44.f};
            std::shared_ptr<Cube> floor = world.Spawn<Cube>(floorPosition, floorRotation, floorScale);
            floor->SetName("Floor");
        }

        void SandboxSceneSpawner::SpawnLights(World& world, GameObject& camera)
        {
            int lightIndex = 0;
            for(int x = 0; x < 1; x++)
            {
                for(int z = 0; z < 2; z++)
                {
                    ...
                    auto light = world.Spawn<PointLight>(position);
            ...
            std::shared_ptr<DirectionalLight> mainDirectionalLight = world.Spawn<DirectionalLight>();
            ...
            std::shared_ptr<DirectionalLight> secondaryDirectionalLight = world.Spawn<DirectionalLight>();
            ...
            std::shared_ptr<PointLight> pointLight = world.Spawn<PointLight>(glm::vec3(-1.f, 0.4f, 10.f));
            pointLight->SetName("PointLight" + std::to_string(lightIndex++));
            pointLight->SetColor(glm::vec3(1.f, 0.f, 0.f));
            pointLight->SetRange(60.f);

            std::shared_ptr<SpotLight> spotLight = world.Spawn<SpotLight>(glm::vec3{2.2f, 4.5f, 6.5f}, glm::vec3{-41.f, 37.f, 0.f});
